% 获取单个视频的处理结果
function [result,annotations] = get_result(video_dir,annotation_path,output_path, proposals, time_cost)
result_dir = 'result';
...
if ~exist(fullfile(result_path, result_file_name),'file')  % not done
    [recall,smT_IoU,hit] = cal_recall(ground_truth_info, annotations, proposals);
    output_info = sprintf('Recall: %.2f%% smT_IoU: %.2f%% object_sum: %d time: %.2f s', recall * 100, smT_IoU * 100, size(hit,1),time_cost);
    disp(output_info);
    for i = 1:size(hit,1)
        h = output_info(1,:);
        h(:) = ' ';
